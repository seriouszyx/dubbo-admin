...
func TestDeleteConfig(t *testing.T) {
	err := SetConfig("test_delete", "test_value")
	if err != nil {
		println("set config (test_delete, test_value) error")
		panic(err)
	}
	config, _ := GetConfig("test_delete")
	assert.Equal(t, "test_value", config)
	err = DeleteConfig("test_delete")
	if err != nil {
		println("delete config test_delete error")
		panic(err)
	}
	config, _ = GetConfig("test_delete")
	assert.Equal(t, "", config)

	err = SetConfigWithGroup("test_group", "test_delete", "test_value")
	if err != nil {
		println("set config (test_group, test_delete, test_value) error")
		panic(err)
	}
	config, _ = GetConfigWithGroup("test_group", "test_delete")
	assert.Equal(t, "test_value", config)
	err = DeleteConfigWithGroup("test_group", "test_delete")
	if err != nil {
		println("delete config (test_group, test_delete) error")
		panic(err)
	}
	config, _ = GetConfigWithGroup("test_group", "test_delete")
	assert.Equal(t, "", config)
